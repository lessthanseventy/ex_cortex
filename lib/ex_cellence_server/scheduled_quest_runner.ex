defmodule ExCellenceServer.ScheduledQuestRunner do
  ...
  defp run_quest(quest) do
    ...
    Task.start(fn ->
      ...
      quest_run_fresh = ExCellenceServer.Repo.get!(ExCellenceServer.Quests.QuestRun, quest_run.id)
      Quests.update_quest_run(quest_run_fresh, %{status: status, results: results})

      if status == "complete" do
        ExCellenceServer.LearningLoop.retrospect(quest, %{quest_run_fresh | results: results})
      end
    end)
  end
end
