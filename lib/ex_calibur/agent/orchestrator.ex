defmodule ExCalibur.Agent.Orchestrator do
  @moduledoc """
  Multi-agent orchestrator. Fans out to role variants in parallel,
  collects verdicts, runs consensus, returns decision.
  """

  alias ExCalibur.Agent.Consensus
  alias ExCalibur.Agent.Registry.CodeResource
  alias ExCalibur.Agent.Registry.DBResource
  alias ExCalibur.Agent.Registry.RoleAdapter
  ...
  defp run_agent(spec, input, context, provider) do
    ...
    case ExCalibur.Agent.LLM.chat(provider, config.model, messages) do
      {:ok, text} ->
  ...
  defp resolve_registry_roles do
    if Process.whereis(ExCalibur.Agent.Registry) do
      ExCalibur.Agent.Registry.list(:role, status: :active) ++
        ExCalibur.Agent.Registry.list(:role, status: :shadow)
    else
      []
    end
  end
