defmodule ExCalibur.QuestThrottle do
  @moduledoc """
  ETS-based per-quest cooldown. Prevents the same quest from running more than
  once per cooldown window, even when multiple sources fire simultaneously.
  """

  @table :quest_cooldowns
  # 5 minutes — feeds all share the same prediction quest, so cap at one run per window
