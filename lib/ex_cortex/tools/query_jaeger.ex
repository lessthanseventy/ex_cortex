defmodule ExCortex.Tools.QueryJaeger do
      ...
      parameter_schema: %{
        ...
        "properties" => %{
          "operation" => %{
            "type" => "string",
            "description" =>
              "Filter by operation name. Options: thought.run (daydream runs), thought.step (synapse executions), llm.complete, llm.complete_with_tools, llm.tool_call, or leave empty for all."
          },
          "lookback" => %{
            "type" => "string",
  ...
  def call(params) do
    base_url = Application.get_env(:ex_cortex, :jaeger_url, @default_url)
  ...
  defp summarize_trace(%{"spans" => spans}) when is_list(spans) do
    ...
    key_attrs =
      (root["tags"] || [])
      |> Enum.filter(fn t -> t["key"] in ~w(thought.name llm.model step.name tool.name) end)
