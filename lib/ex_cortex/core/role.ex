defmodule ExCortex.Core.Role do
  @moduledoc """
  ...
  @callback parse_response(text :: String.t(), agent_name :: String.t(), opts :: keyword()) ::
              ExCortex.Core.Verdict.t()

  defmacro __using__(_opts) do
    quote do
      @behaviour ExCortex.Core.Role

      import ExCortex.Core.Role, only: [perspective: 2, system_prompt: 1]
      ...
      @before_compile ExCortex.Core.Role
    end
  end
  ...
  defmacro __before_compile__(env) do
    ...
    quote do
      ...
      def parse_response(text, agent_name) do
        ExCortex.Core.Role.DefaultParser.parse(text, agent_name, __MODULE__, [])
      end

      def parse_response(text, agent_name, opts) do
        ExCortex.Core.Role.DefaultParser.parse(text, agent_name, __MODULE__, opts)
      end
