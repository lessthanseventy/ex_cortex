defmodule ExCortex.AppTelemetry do
  @moduledoc """
  In-process telemetry ring buffer for the ExCortex app.
  ...
  - Daydream outcomes from PubSub — last 50

  Instrumentation points:
  - `ExCortex.LLM.Ollama` calls `record_llm_call/3` and `record_circuit_breaker/1`
  - Logger handler captures :warning/:error level events
  - PubSub subscription receives `{:daydream_completed, daydream}` events
  """
  ...
  def init(state) do
    ...
    :logger.add_handler(:app_telemetry, ExCortex.AppTelemetry.LoggerHandler, %{
      config: %{pid: self()},
      level: :warning
    })
  ...
  def handle_info({:daydream_completed, daydream}, state) do
    ...
    {:noreply, %{state | daydreams: add_to_ring(state.daydreams, entry, @max_daydreams)}}
  rescue
    _ -> {:noreply, state}
  end
  ...
  defp format_recent(%__MODULE__{} = state, cutoff) do
    sections =
      Enum.reject(
        [
          format_daydreams(state.daydreams, cutoff),
  ...
  defp format_daydreams([], _cutoff), do: ""

  defp format_daydreams(daydreams, cutoff) do
    recent = Enum.filter(daydreams, &(&1.timestamp >= cutoff))
    if recent == [], do: "", else: do_format_daydreams(recent)
  end

  defp do_format_daydreams(daydreams) do
    by_status = Enum.group_by(daydreams, & &1.status)
    complete = length(Map.get(by_status, "complete", []))
    gated = Map.get(by_status, "gated", [])
    failed = Map.get(by_status, "failed", [])

    summary = "Daydreams: #{complete} complete"
  ...
  defp extract_failed_synapse(_), do: nil
end
