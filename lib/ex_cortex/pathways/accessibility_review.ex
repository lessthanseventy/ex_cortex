defmodule ExCortex.Pathways.AccessibilityReview do
  @moduledoc """
  Accessibility review pipeline pathway.
  ...
  def synapse_definitions do
    [
      %{
        ...
        description: "Quick automated accessibility check by apprentice neurons",
        status: "active",
        trigger: "scheduled",
        schedule: "@hourly",
        roster: [%{"who" => "apprentice", "when" => "on_trigger", "how" => "solo"}],
        source_ids: [],
        context_providers: [%{"type" => "memory", "tags" => ["a11y"], "limit" => 3, "sort" => "importance"}],
        escalate: true,
  ...
  def rumination_definitions do
    [
      %{
        ...
        schedule: "@monthly",
        steps: [
          %{"thought_name" => "WCAG Hourly Scan", "flow" => "always"},
          %{"thought_name" => "Full Accessibility Audit", "flow" => "on_flag"},
          %{"thought_name" => "Post A11y Alert", "flow" => "on_flag"},
          %{"thought_name" => "A11y Knowledge Synthesis", "flow" => "always"}
        ],
        source_ids: []
      }
    ]
