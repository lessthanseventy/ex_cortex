defmodule ExCortexWeb.LodgeLive do
  @moduledoc false
  ...
  defp mount_lodge(socket) do
    ...
    has_members =
      Repo.exists?(from(r in Neuron, where: r.type == "role"))
  ...
  defp load_cards(socket) do
    ...
    cards = Signals.list_signals(opts)
  ...
  def handle_event("toggle_checklist_item", %{"card-id" => id, "index" => idx}, socket) do
    card = Signals.get_signal!(id)
    Signals.toggle_checklist_item(card, String.to_integer(idx))
    {:noreply, load_cards(socket)}
  end
  ...
  def handle_event("reject_proposal", %{"card-id" => id}, socket) do
    ...
    if proposal_id do
      ...
      if proposal, do: ExCortex.Thoughts.reject_proposal(proposal)
    end
    ...
    <div class="space-y-6">
      <div>
        ...
        <p class="text-muted-foreground mt-1.5">
          Your cluster's dashboard — pinned cards, thought output, and notes.
        </p>
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <%= for entry <- @dev_team_status do %>
              <div class="flex items-center gap-1.5 text-xs rounded-full border bg-background px-2.5 py-1">
                ...
                <span class="font-medium">{entry.thought.name}</span>
        ...
        <%= if @feed_cards == [] and @pinned_cards == [] do %>
          <div class="rounded-lg border p-8 text-center">
            <p class="text-muted-foreground text-sm">
              No cards yet. Cards appear here when thoughts run, or you can create one above.
            </p>
            <p class="text-xs text-muted-foreground mt-2">
              Set up thoughts from the <a href="/thoughts" class="underline text-primary">Thoughts</a>
              page.
            </p>
          </div>
        <% else %>
  ...
  defp maybe_approve_proposal(proposal_id) do
    ...
    if proposal do
      ExCortex.Thoughts.approve_proposal(proposal)

      if proposal.type == "tool_action" do
        ExCortex.Thoughts.execute_tool_proposal(proposal)
      end
    end
  end
  ...
  defp update_action_list_item(card_id, item_id, status) do
    ...
    Signals.update_signal(card, %{metadata: Map.put(card.metadata, "items", updated_items)})
  end
  ...
  defp load_dev_team_status do
    ...
    Enum.map(thoughts, fn q ->
      ...
      %{thought: q, last_run: run, artifact_count: count_run_artifacts(run)}
    end)
  end
