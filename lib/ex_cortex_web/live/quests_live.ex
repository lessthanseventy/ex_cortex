defmodule ExCortexWeb.QuestsLive do
  ...
  defp mount_quests(socket) do
    ...
    if connected?(socket) do
      Phoenix.PubSub.subscribe(ExCortex.PubSub, "step_runs")
    end

    steps = Thoughts.list_synapses()
    thoughts = Thoughts.list_thoughts()

    sources = ExCortex.Repo.all(from(s in Sense, order_by: [asc: s.inserted_at]))

    trigger_previews =
      thoughts
      |> Map.new(fn c -> {"thought-#{c.id}", c.trigger} end)
      |> Map.put("new-thought", "manual")

    schedule_mode_previews = build_schedule_mode_previews(thoughts)
  ...
  def handle_params(_params, _url, socket) do
    {:noreply, assign(socket, page_title: "Thoughts")}
  end
  ...
  @impl true
  def handle_event("toggle_expand", %{"id" => "thought-" <> quest_id_str = id}, socket) do
    expanded =
    ...
    daydreams =
      if MapSet.member?(socket.assigns.expanded, id) do
        ...
        thought = Thoughts.get_thought!(String.to_integer(quest_id_str))
        Map.put(socket.assigns.daydreams, quest_id_str, Thoughts.list_daydreams(thought))
      end

    {:noreply, assign(socket, expanded: expanded, daydreams: daydreams)}
  end
  ...
  @impl true
  def handle_event("preview_quest_trigger", %{"quest_id" => id, "thought" => %{"trigger" => t}}, socket) do
    {:noreply, assign(socket, trigger_previews: Map.put(socket.assigns.trigger_previews, "thought-#{id}", t))}
  end

  def handle_event("preview_quest_trigger", _params, socket), do: {:noreply, socket}

  @impl true
  def handle_event("preview_new_quest_trigger", %{"thought" => %{"trigger" => t}}, socket) do
    {:noreply, assign(socket, trigger_previews: Map.put(socket.assigns.trigger_previews, "new-thought", t))}
  end
  ...
  def handle_event("move_quest_step_up", %{"quest_id" => id, "index" => idx_str}, socket) do
    ...
    Thoughts.update_thought(thought, %{steps: new_steps})
  ...
  def handle_event("remove_quest_step", %{"quest_id" => id, "index" => idx_str}, socket) do
    idx = String.to_integer(idx_str)
    thought = Thoughts.get_thought!(String.to_integer(id))
    new_steps = List.delete_at(thought.steps, idx)
    Thoughts.update_thought(thought, %{steps: new_steps})
    {:noreply, assign_quests(socket)}
  end
  ...
  def handle_event("add_quest_step", %{"quest_id" => id, "step_id" => step_id}, socket) do
    ...
    Thoughts.update_thought(thought, %{steps: thought.steps ++ [new_step]})
    {:noreply, assign_quests(socket)}
  end

  @impl true
  def handle_event("create_quest", %{"thought" => params}, socket) do
    ...
    case Thoughts.create_thought(attrs) do
      {:ok, _} ->
        thoughts = Thoughts.list_thoughts()
        previews = rebuild_trigger_previews(socket.assigns.steps, thoughts, socket.assigns.trigger_previews)
        schedule_mode_previews = build_schedule_mode_previews(thoughts)
      ...
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Failed to create thought")}
    end
  end

  @impl true
  def handle_event("update_quest", %{"thought" => params, "quest_id" => id}, socket) do
    thought = Thoughts.get_thought!(String.to_integer(id))
    attrs = build_quest_attrs(params)

    case Thoughts.update_thought(thought, attrs) do
      {:ok, _} ->
        thoughts = Thoughts.list_thoughts()
        previews = rebuild_trigger_previews(socket.assigns.steps, thoughts, socket.assigns.trigger_previews)
        schedule_mode_previews = build_schedule_mode_previews(thoughts)
        ...
        {:noreply,
         assign(socket, thoughts: thoughts, trigger_previews: previews, schedule_mode_previews: schedule_mode_previews)}

      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Failed to update thought")}
    end
  end
  ...
  def handle_event("toggle_quest_status", %{"id" => id}, socket) do
    thought = Thoughts.get_thought!(String.to_integer(id))
    new_status = if thought.status == "active", do: "paused", else: "active"
    Thoughts.update_thought(thought, %{status: new_status})
    {:noreply, assign_quests(socket)}
  end
  ...
  def handle_event("run_quest_now", %{"id" => id}, socket) do
    thought = Thoughts.get_thought!(String.to_integer(id))
    run_id = "thought-#{thought.id}"

    running = Map.put(socket.assigns.running, run_id, %{status: "running", result: nil})
    parent = self()

    Task.start(fn ->
      result = ExCortex.Thoughts.Runner.run(thought, "")
      send(parent, {:daydream_complete, run_id, result})
    end)
  ...
  def handle_event("update_step_grimoire", %{"step_id" => step_id, "tags" => tags_str}, socket) do
    step = Thoughts.get_synapse!(String.to_integer(step_id))
    tags = tags_str |> String.split(",") |> Enum.map(&String.trim/1) |> Enum.reject(&(&1 == ""))

    new_providers =
      case tags do
        [] ->
          Enum.reject(step.context_providers, &(&1["type"] == "memory"))

        tags ->
          existing = Enum.reject(step.context_providers, &(&1["type"] == "memory"))
          existing ++ [%{"type" => "memory", "tags" => tags, "limit" => 10, "sort" => "top"}]
      end

    Thoughts.update_synapse(step, %{context_providers: new_providers})
    {:noreply, assign_steps(socket)}
  end

  def handle_event("update_step_output_type", %{"step_id" => step_id, "output_type" => output_type}, socket) do
    step = Thoughts.get_synapse!(String.to_integer(step_id))
    Thoughts.update_synapse(step, %{output_type: output_type})
    {:noreply, assign_steps(socket)}
  end
  ...
  def handle_event("update_step_lore_tags", %{"step_id" => step_id, "lore_tags" => tags_str}, socket) do
    step = Thoughts.get_synapse!(String.to_integer(step_id))
    tags = tags_str |> String.split(",") |> Enum.map(&String.trim/1) |> Enum.reject(&(&1 == ""))
    Thoughts.update_synapse(step, %{lore_tags: tags})
    {:noreply, assign_steps(socket)}
  end

  @impl true
  def handle_info({:daydream_complete, run_id, result}, socket) do
    {status, results} =
      case result do
  ...
  def handle_info(:refresh, socket) do
    {:noreply, assign(socket, thoughts: Thoughts.list_thoughts(), steps: Thoughts.list_synapses())}
  end
    ...
    <div class="space-y-10">
      <div>
        ...
        <p class="text-sm text-muted-foreground">
          {length(@thoughts)} thoughts · {Enum.count(@thoughts, &(&1.trigger == "scheduled"))} scheduled · {Enum.count(
            @thoughts,
            &(&1.trigger == "source")
          )} source-triggered
        </p>
      ...
      <div>
        <div class="space-y-3">
          <.quest_card_comp
            ...
            steps={@steps}
            sources={@sources}
            expanded={MapSet.member?(@expanded, "thought-#{thought.id}")}
            trigger_preview={@trigger_previews["thought-#{thought.id}"] || thought.trigger}
            schedule_mode_previews={@schedule_mode_previews}
            run_state={@running["thought-#{thought.id}"]}
            runs={@daydreams[to_string(thought.id)] || []}
          />
          <%= if @thoughts == [] do %>
            <div class="text-center py-12 text-muted-foreground">
              <p class="text-sm">
                No thoughts yet. Install one from the Thought Board above, or create your own.
              </p>
            </div>
          <% end %>
        </div>
      ...
      <div>
        <div class="mb-4">
          <h2 class="text-lg font-semibold">Thought Templates</h2>
          <p class="text-sm text-muted-foreground">
            Pre-built thoughts ready to install. Each adds a workflow to your cluster.
          </p>
        </div>
        ...
        <%= if @board_tab == "custom" do %>
          <.new_quest_form_comp
            steps={@steps}
            sources={@sources}
            trigger_preview={@trigger_previews["new-thought"] || "manual"}
  ...
  attr :namespace, :string, default: "thought"
  ...
  defp new_quest_form_comp(assigns) do
    ...
    <div class="border rounded-lg border-dashed p-4">
      <form phx-submit="create_quest" phx-change="preview_new_quest_trigger" class="space-y-3">
        ...
        <div>
          <label for="thought-trigger-new" class="text-sm font-medium">Trigger</label>
          <select
            id="thought-trigger-new"
          ...
          >
            ...
            <option value="memory">Grimoire</option>
        ...
        <%= if @trigger_preview == "scheduled" do %>
          <.schedule_picker
            namespace="thought"
            id_prefix="new-thought"
            mode={Map.get(@schedule_mode_previews, "new-thought", "interval")}
            schedule=""
          />
        <% end %>
        ...
        <%= if @trigger_preview == "source" do %>
          <.source_picker sources={@sources} namespace="thought" selected_ids={[]} />
        <% end %>
        <%= if @trigger_preview == "memory" do %>
          <div>
            <label class="text-sm font-medium">Trigger on tags</label>
            <input
              type="text"
              name="thought[lore_trigger_tags]"
              value=""
        ...
        <% end %>
        <%= if @trigger_preview == "cortex" do %>
          <div class="space-y-2">
            <div>
              <label class="text-sm font-medium">Card types (optional)</label>
              <input
                type="text"
                name="thought[lodge_trigger_types]"
                value=""
                placeholder="todo, checklist, note… (empty = all)"
                class="w-full h-9 text-sm border border-input rounded-md px-3 bg-background focus:outline-none focus:ring-1 focus:ring-ring"
              />
              <p class="text-xs text-muted-foreground mt-1">
                Fires when a signal card of these types is created.
              </p>
            </div>
            <div>
              ...
              <input
                type="text"
                name="thought[lodge_trigger_tags]"
                value=""
        ...
        <div>
          ...
          <select
            name="thought[step_ids][]"
            multiple
            class="w-full text-sm border rounded px-2 py-1 bg-background h-24"
          >
            <%= for thought <- @steps do %>
              <option value={thought.id}>{thought.name}</option>
            <% end %>
          </select>
        ...
        <div class="flex justify-end gap-2">
          ...
          <.button type="submit" size="sm">Create Thought</.button>
        </div>
      </form>
    </div>
    """
  ...
  attr :thought, :map, required: true
  attr :steps, :list, required: true
  attr :sources, :list, default: []
      ...
      <div class="flex items-stretch gap-3 px-5">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <span class="font-medium">{@thought.name}</span>
            <span class="text-xs text-muted-foreground ml-2">
              {length(@thought.steps)} steps
            </span>
            <%= if @thought.trigger == "memory" && @thought.lore_trigger_tags != [] do %>
              <span class="text-xs text-muted-foreground ml-1">
                · listening for:
                <span class="font-mono">{Enum.join(@thought.lore_trigger_tags, ", ")}</span>
              </span>
            <% end %>
            <%= if @thought.trigger == "memory" && (@thought.lore_trigger_tags == nil || @thought.lore_trigger_tags == []) do %>
              <span class="text-xs text-muted-foreground ml-1">
                · listening for all Grimoire entries
              </span>
            <% end %>
            <%= if @thought.trigger == "source" and @thought.source_ids != [] do %>
              <p class="text-xs text-muted-foreground">
                Triggered by: {Enum.map_join(@thought.source_ids, ", ", fn sid ->
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          <.button
            ...
            phx-value-id={@thought.id}
          >
            {if @thought.status == "active", do: "Pause", else: "Resume"}
          </.button>
          <.button
            size="sm"
            variant="ghost"
      ...
      <%= if @expanded do %>
        <div class="border-t px-5 py-5">
          <form phx-submit="update_quest" phx-change="preview_quest_trigger" class="space-y-4">
            <input type="hidden" name="quest_id" value={@thought.id} />
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div class="space-y-1.5">
                <label for={"qname-form-#{@thought.id}"} class="text-sm font-medium">Name</label>
                <.input
                  id={"qname-form-#{@thought.id}"}
                  type="text"
                  name="thought[name]"
                  value={@thought.name}
                />
              ...
              <div class="space-y-1.5">
                <label for={"qdesc-form-#{@thought.id}"} class="text-sm font-medium">
                  Description
                </label>
                <.input
                  id={"qdesc-form-#{@thought.id}"}
                  type="text"
                  name="thought[description]"
            ...
            <div class="space-y-1.5">
              <label for={"qtrigger-form-#{@thought.id}"} class="text-sm font-medium">Trigger</label>
              <select
                id={"qtrigger-form-#{@thought.id}"}
                name="thought[trigger]"
                class="w-full h-9 text-sm border border-input rounded-md px-3 bg-background focus:outline-none focus:ring-1 focus:ring-ring"
              >
                <option value="manual" selected={@thought.trigger == "manual"}>Manual</option>
                <option value="source" selected={@thought.trigger == "source"}>Library Source</option>
                <option value="scheduled" selected={@thought.trigger == "scheduled"}>
                  Scheduled
                </option>
                <option value="once" selected={@thought.trigger == "once"}>
                  Once (specific time)
                </option>
                <option value="memory" selected={@thought.trigger == "memory"}>
                  Grimoire
                </option>
                <option value="cortex" selected={@thought.trigger == "cortex"}>
                  Cortex
                </option>
              </select>
            </div>
            <%= if @trigger_preview == "scheduled" do %>
              <.schedule_picker
                namespace="thought"
                id_prefix={"thought-#{@thought.id}"}
                mode={
                  Map.get(
                    @schedule_mode_previews,
                    "thought-#{@thought.id}",
                    detect_schedule_mode(@thought.schedule)
                  )
                }
                schedule={@thought.schedule}
              />
            <% end %>
            <%= if @trigger_preview == "once" do %>
              <div>
            ...
            <%= if @trigger_preview == "source" do %>
              <.source_picker
                sources={@sources}
                namespace="thought"
                selected_ids={@thought.source_ids}
              />
            <% end %>
            <%= if @trigger_preview == "memory" do %>
              <div>
                ...
                <input
                  type="text"
                  name="thought[lore_trigger_tags]"
                  value={Enum.join(@thought.lore_trigger_tags || [], ", ")}
                  placeholder="journal, decisions… (empty = all entries)"
                  class="w-full h-9 text-sm border border-input rounded-md px-3 bg-background focus:outline-none focus:ring-1 focus:ring-ring"
                />
                <p class="text-xs text-muted-foreground mt-1">
              ...
              <div class="space-y-2">
                <div>
                  ...
                  <input
                    type="text"
                    name="thought[lodge_trigger_types]"
                    value={Enum.join(@thought.lodge_trigger_types || [], ", ")}
                    placeholder="todo, checklist, note… (empty = all)"
                    class="w-full h-9 text-sm border border-input rounded-md px-3 bg-background focus:outline-none focus:ring-1 focus:ring-ring"
                  />
                  <p class="text-xs text-muted-foreground mt-1">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="thought[lodge_trigger_tags]"
                    value={Enum.join(@thought.lodge_trigger_tags || [], ", ")}
                    placeholder="urgent, todo… (empty = all)"
                    class="w-full h-9 text-sm border border-input rounded-md px-3 bg-background focus:outline-none focus:ring-1 focus:ring-ring"
                  />
                </div>
          ...
          <div class="space-y-1.5 mt-4">
            ...
            <% else %>
              <div class="rounded-md border divide-y divide-border">
                <%= for {step, idx} <- Enum.with_index(@thought.steps) do %>
                  <% step_struct =
                    Enum.find(@steps, &(to_string(&1.id) == to_string(step["step_id"]))) %>
                  <% sub_tags =
                    if step_struct do
                      case Enum.find(step_struct.context_providers || [], &(&1["type"] == "memory")) do
                        nil -> []
                        p -> p["tags"] || []
                      end
                    else
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div class="flex items-center gap-1 shrink-0">
                        ...
                        <button
                          ...
                          phx-value-quest_id={@thought.id}
                          phx-value-index={idx}
                          disabled={idx == length(@thought.steps) - 1}
  ...
  defp rebuild_trigger_previews(steps, thoughts, existing) do
    existing
    |> Map.merge(Map.new(steps, fn q -> {"step-#{q.id}", q.trigger} end))
    |> Map.merge(Map.new(thoughts, fn c -> {"thought-#{c.id}", c.trigger} end))
  end
  ...
  defp build_schedule_mode_previews(thoughts) do
    thoughts
    |> Map.new(fn q -> {"thought-#{q.id}", detect_schedule_mode(q.schedule)} end)
    |> Map.put("new-thought", "interval")
  end
  ...
  defp assign_quests(socket) do
    assign(socket, thoughts: Thoughts.list_thoughts())
  end

  defp assign_steps(socket) do
    assign(socket, steps: Thoughts.list_synapses())
  end
  ...
  defp source_label(%Sense{name: name}) when is_binary(name) and name != "", do: name

  defp source_label(%Sense{book_id: book_id}) when is_binary(book_id) do
  ...
  defp source_label(%Sense{source_type: type}), do: String.capitalize(type) <> " source"

  defp trigger_label("source"), do: "Library Source"
  defp trigger_label("memory"), do: "Grimoire"
