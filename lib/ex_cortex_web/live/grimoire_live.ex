defmodule ExCortexWeb.GrimoireLive do
  ...
  defp mount_grimoire(socket) do
    ...
    run_stats = load_run_stats(thoughts)
    telemetry = load_telemetry(thoughts, run_stats)

    {:ok,
     assign(socket,
       page_title: "Grimoire",
       active_tab: "thought-log",
       thoughts: thoughts,
       run_stats: run_stats,
       telemetry: telemetry,
  ...
  def handle_info(:refresh, socket) do
    thoughts = Thoughts.list_thoughts()
    run_stats = load_run_stats(thoughts)
    {:noreply, assign(socket, thoughts: thoughts, run_stats: run_stats)}
  end
  ...
  defp load_quest_data(socket, quest_id) do
    ...
    lore_entries = Memory.list_engrams(thought_id: quest_id)

    assign(socket,
      selected_quest_id: quest_id,
      selected_quest: thought,
      daydreams: daydreams,
      quest_lore_entries: lore_entries
    )
  end

  defp load_run_stats(thoughts) do
    quest_ids = Enum.map(thoughts, & &1.id)
    ...
    if quest_ids == [] do
      ...
      runs =
        Repo.all(
          from r in Daydream,
            where: r.thought_id in ^quest_ids,
            select: {r.thought_id, r.status, r.inserted_at}
        )

      runs
      |> Enum.group_by(&elem(&1, 0))
      |> Map.new(fn {quest_id, daydreams} -> {quest_id, build_quest_stats(daydreams)} end)
    end
  end

  defp build_quest_stats(daydreams) do
    total = length(daydreams)
    complete = Enum.count(daydreams, fn {_, status, _} -> status == "complete" end)
    failed = Enum.count(daydreams, fn {_, status, _} -> status == "failed" end)
      ...
      <% else %>
        <div>
          ...
          <p class="text-muted-foreground mt-1.5">
            Thought log and telemetry for your cluster's missions.
          </p>
          <p class="text-sm text-muted-foreground">
            {length(@thoughts)} thoughts tracked · {Enum.sum(
              Enum.map(@run_stats, fn {_id, s} -> s.total end)
            )} runs total
          </p>
        </div>

        <.tabs id="grimoire-tabs" default="thought-log">
          <.tabs_list>
            <.tabs_trigger value="thought-log">Thought Log</.tabs_trigger>
            <.tabs_trigger value="telemetry">Telemetry</.tabs_trigger>
          </.tabs_list>

          <.tabs_content value="thought-log">
            <div class="space-y-4 mt-4">
              <%= if @thoughts == [] do %>
                <div class="rounded-lg border p-8 text-center">
                  <p class="text-muted-foreground text-sm">
                    No thoughts yet. Create one from the
                    <a href="/thoughts" class="underline text-primary">Thoughts</a>
                    page. Once thoughts run, their history and engrams show up here.
                  </p>
                </div>
              <% else %>
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  <%= for thought <- @thoughts do %>
                    <.quest_card thought={thought} stats={Map.get(@run_stats, thought.id)} />
                  <% end %>
                </div>
              <% end %>
            </div>
            ...
            <div class="space-y-4 mt-4">
              ...
              <div class="grid gap-4 grid-cols-2 md:grid-cols-4">
                ...
                <.card>
                  <.card_content class="pt-6">
                    ...
                    <p class="text-xs text-muted-foreground">daydreams</p>
                  </.card_content>
                </.card>
                <.card>
                  <.card_content class="pt-6">
                    <div class="text-2xl font-bold">{@telemetry.lore_count}</div>
                    <p class="text-xs text-muted-foreground">engrams</p>
                  </.card_content>
                </.card>
                <.card>
                  <.card_content class="pt-6">
                    <div class="text-2xl font-bold">{@telemetry.card_count}</div>
                    <p class="text-xs text-muted-foreground">signal cards</p>
                  </.card_content>
                </.card>
              </div>
              ...
              <.card>
                <.card_header>
                  ...
                  <.card_description>
                    Data sources feeding your clusters.
                  </.card_description>
                </.card_header>
                <.card_content>
              ...
              <%= if @telemetry.dev_team != [] do %>
                <.card>
                  <.card_header>
                    ...
                    <.card_description>
                      Dev Team thought status — last run and artifacts produced.
                    </.card_description>
                  </.card_header>
                  <.card_content>
                    <div class="space-y-2">
                        ...
                        <div class="flex items-start gap-3 text-sm py-1 border-b last:border-0">
                          <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                              <span class="font-medium truncate">{entry.thought.name}</span>
    ...
    >
      <.card_header class="pb-3">
        <div class="flex items-center justify-between gap-2">
          <.card_title class="truncate">{@thought.name}</.card_title>
          <.badge variant={if @thought.status == "active", do: "default", else: "secondary"}>
            {@thought.status}
          </.badge>
        </div>
        <%= if @thought.description do %>
          <.card_description class="line-clamp-2">{@thought.description}</.card_description>
        <% end %>
      </.card_header>
      <.card_content class="pt-0">
        <div class="flex items-center gap-3 text-xs text-muted-foreground">
          <span>Trigger: {@thought.trigger}</span>
          <span>·</span>
    ...
    <div class="space-y-6">
      ...
      <.card>
        <.card_header>
          <div class="flex items-center justify-between gap-3">
            <.card_title class="text-2xl">{@thought.name}</.card_title>
            <.badge variant={if @thought.status == "active", do: "default", else: "secondary"}>
              {@thought.status}
            </.badge>
          </div>
          <%= if @thought.description do %>
            <.card_description>{@thought.description}</.card_description>
          <% end %>
        </.card_header>
        <.card_content>
          <div class="flex items-center gap-4 text-sm text-muted-foreground">
            <div>
              <span class="font-medium text-foreground">Trigger:</span> {@thought.trigger}
            </div>
            <div>
      ...
      <.card>
        <.card_header>
          ...
          <.card_description>Recent daydreams</.card_description>
        </.card_header>
        <.card_content>
          <%= if @runs == [] do %>
            <p class="text-sm text-muted-foreground">
              No runs yet. Run this thought from the
              <a href="/thoughts" class="underline text-primary">Thoughts</a>
          ...
          <% else %>
            <div class="space-y-3">
              <%= for run <- @runs do %>
                <% all_tool_calls =
                  run.synapse_results |> Map.values() |> Enum.flat_map(&Map.get(&1, "tool_calls", [])) %>
                <% artifacts =
                  all_tool_calls
      ...
      <.card>
        <.card_header>
          <.card_title>engrams</.card_title>
          <.card_description>Knowledge generated by this thought</.card_description>
        </.card_header>
        <.card_content>
          <%= if @lore_entries == [] do %>
            <p class="text-sm text-muted-foreground">
              No engrams yet. Memory is written by thought steps as they process input.
            </p>
          <% else %>
            <div class="space-y-3">
  ...
  defp load_telemetry(_quests, run_stats) do
    ...
    lore_count = Repo.aggregate(Engram, :count)
    card_count = Repo.aggregate(Signal, :count)

    source_counts =
      from(s in Sense, group_by: s.status, select: {s.status, count(s.id)})
      |> Repo.all()
      |> Map.new()
    ...
    ollama = ExCortex.OllamaCache.get_status()
  ...
  defp load_dev_team_status do
    thoughts =
      Repo.all(from q in Thought, where: q.name in @dev_team_quest_names, order_by: q.name)
    ...
    last_runs =
      ...
      |> Repo.all()
      |> Enum.group_by(& &1.thought_id)
      |> Map.new(fn {id, [latest | _]} -> {id, latest} end)

    Enum.map(thoughts, fn q ->
      ...
      %{thought: q, last_run: run, tool_call_count: length(tool_calls), artifacts: artifacts}
    end)
  end
