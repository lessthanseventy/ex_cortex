...
**Architecture:** Heralds are named credential configs stored in a new `heralds` DB table. Quest `output_type` is extended with six herald type values. When a herald-type quest runs, `QuestRunner` generates LLM content via the existing `run_artifact/2` path, then dispatches to `ExCalibur.Heralds.deliver/3` which routes to a per-type module. The Library page gains a Heralds tab for CRUD. Campaigns wire verdict → herald delivery via existing `flow: "on_flag"` steps.
...
- Create: `lib/ex_calibur/heralds/herald.ex`
- Create: `lib/ex_calibur/heralds.ex`
- Create: `test/ex_calibur/heralds_test.exs`

**Step 1: Write failing tests**

```elixir
# test/ex_calibur/heralds_test.exs
...
```bash
tmux-cli send 'cd /home/andrew/projects/ex_calibur && mix test test/ex_calibur/heralds_test.exs 2>&1 | tail -5' --pane=main:1.3
```
...
```elixir
# lib/ex_calibur/heralds/herald.ex
...
```bash
tmux-cli send 'mix test test/ex_calibur/heralds_test.exs 2>&1 | tail -5' --pane=main:1.3
```
...
```bash
tmux-cli send 'git add priv/repo/migrations/20260308310000_create_heralds.exs lib/ex_calibur/heralds/herald.ex lib/ex_calibur/heralds.ex test/ex_calibur/heralds_test.exs && git commit -m "feat: heralds schema, context, and migration"' --pane=main:1.3
```
...
**Files:**
- Create: `lib/ex_calibur/heralds/slack.ex`
- Create: `lib/ex_calibur/heralds/webhook.ex`
- Create: `lib/ex_calibur/heralds/github_issue.ex`
- Create: `lib/ex_calibur/heralds/github_pr.ex`
- Create: `lib/ex_calibur/heralds/email.ex`
- Create: `lib/ex_calibur/heralds/pager_duty.ex`
...
```elixir
# lib/ex_calibur/heralds/slack.ex
...
```elixir
# lib/ex_calibur/heralds/webhook.ex
...
```elixir
# lib/ex_calibur/heralds/github_issue.ex
...
```elixir
# lib/ex_calibur/heralds/github_pr.ex
...
```elixir
# lib/ex_calibur/heralds/email.ex
...
```elixir
# lib/ex_calibur/heralds/pager_duty.ex
...
```bash
tmux-cli send 'git add lib/ex_calibur/heralds/ && git commit -m "feat: herald delivery modules — slack, webhook, github, email, pagerduty"' --pane=main:1.3
```
...
- Modify: `test/ex_calibur_web/live/quests_live_test.exs` (no changes needed, existing tests cover non-herald paths)
- Create: `test/ex_calibur/quest_runner_herald_test.exs`

**Step 1: Write failing herald runner tests**

```elixir
# test/ex_calibur/quest_runner_herald_test.exs
...
```bash
tmux-cli send 'mix test test/ex_calibur/quest_runner_herald_test.exs 2>&1 | tail -8' --pane=main:1.3
```
...
Add `herald_name` field and extend `output_type` validation in `lib/ex_calibur/quests/quest.ex`:
...
In `lib/ex_calibur/quest_runner.ex`, add before the existing `def run(quest, input_text) when is_struct(quest)` clause:
...
```bash
tmux-cli send 'git add priv/repo/migrations/20260308320000_add_herald_name_to_quests.exs lib/ex_calibur/quests/quest.ex lib/ex_calibur/quest_runner.ex test/ex_calibur/quest_runner_herald_test.exs && git commit -m "feat: herald output types in quest schema and runner"' --pane=main:1.3
```
...
- Modify: `test/ex_calibur_web/live/library_live_test.exs`

**Step 1: Read the current library test file**

Read `test/ex_calibur_web/live/library_live_test.exs` in full. Understand the existing test structure before adding to it.

**Step 2: Add herald tab tests**

Add to `test/ex_calibur_web/live/library_live_test.exs`:
...
```bash
tmux-cli send 'mix test test/ex_calibur_web/live/library_live_test.exs 2>&1 | tail -10' --pane=main:1.3
```
...
In `lib/ex_calibur_web/live/library_live.ex`:
...
```bash
tmux-cli send 'git add lib/ex_calibur_web/live/library_live.ex test/ex_calibur_web/live/library_live_test.exs && git commit -m "feat: heralds tab in Library — create and delete delivery integrations"' --pane=main:1.3
```
...
- Modify: `test/ex_calibur_web/live/quests_live_test.exs`

**Step 1: Read the current quest test file in full**

Read `test/ex_calibur_web/live/quests_live_test.exs`. Understand what's already tested.

**Step 2: Add tests for herald output type in quest form**

Add to `test/ex_calibur_web/live/quests_live_test.exs`:
...
```bash
tmux-cli send 'mix test test/ex_calibur_web/live/quests_live_test.exs 2>&1 | tail -10' --pane=main:1.3
```
...
<%= if @output_preview in ~w(slack webhook github_issue github_pr email pagerduty) do %>
  <div>
    ...
    <% type_heralds = ExCalibur.Heralds.list_by_type(@output_preview) %>
    <%= if type_heralds == [] do %>
...
```bash
tmux-cli send 'git add lib/ex_calibur_web/live/quests_live.ex test/ex_calibur_web/live/quests_live_test.exs && git commit -m "feat: herald output types in quest form with herald_name select and badge"' --pane=main:1.3
```
