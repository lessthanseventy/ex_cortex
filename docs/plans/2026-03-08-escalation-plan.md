...
**Files:**
- Create: `lib/ex_cellence_server/claude_client.ex`
- Create: `test/ex_cellence_server/claude_client_test.exs`

**Step 1: Write the failing test**

```elixir
# test/ex_cellence_server/claude_client_test.exs
defmodule ExCellenceServer.ClaudeClientTest do
...
```bash
cd /home/andrew/projects/ex_cellence_server && mix test test/ex_cellence_server/claude_client_test.exs
```
...
```elixir
# lib/ex_cellence_server/claude_client.ex
defmodule ExCellenceServer.ClaudeClient do
  ...
  def call(tier, system_prompt, user_message) do
    api_key = Application.get_env(:ex_cellence_server, :anthropic_api_key) ||
...
```bash
mix test test/ex_cellence_server/claude_client_test.exs
```
...
```bash
git add lib/ex_cellence_server/claude_client.ex test/ex_cellence_server/claude_client_test.exs
...
**Files:**
- Create: `lib/ex_cellence_server/quest_runner.ex`
- Create: `test/ex_cellence_server/quest_runner_test.exs`

**Step 1: Write the failing tests**

```elixir
# test/ex_cellence_server/quest_runner_test.exs
defmodule ExCellenceServer.QuestRunnerTest do
...
```bash
mix test test/ex_cellence_server/quest_runner_test.exs
```
...
```elixir
# lib/ex_cellence_server/quest_runner.ex
...
```bash
git add lib/ex_cellence_server/quest_runner.ex test/ex_cellence_server/quest_runner_test.exs
...
```bash
mix compile --warnings-as-errors && mix test test/ex_cellence_server_web/live/quests_live_test.exs
```
...
```elixir
config :ex_cellence_server, :anthropic_api_key,
  System.get_env("ANTHROPIC_API_KEY")
```
...
```elixir
# config :ex_cellence_server, :anthropic_api_key, "sk-ant-..."
```
