...
**Tech Stack:** Phoenix LiveView, SaladUI (Card, Badge, Button, Input, Textarea), Tailwind CSS, Ecto/ResourceDefinition schema, ExCellenceServer.Members.Member built-in catalog
...
### Built-in members — `lib/ex_cellence_server/members/member.ex`
...
### Current test file
`test/ex_cellence_server_web/live/members_live_test.exs` — currently has one trivial test. You will rewrite it.
...
**Files:**
- Modify: `lib/ex_cellence_server_web/live/members_live.ex`
...
Replace contents of `test/ex_cellence_server_web/live/members_live_test.exs`:
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix test test/ex_cellence_server_web/live/members_live_test.exs 2>&1 | tail -20
```
...
Add these private functions to `lib/ex_cellence_server_web/live/members_live.ex`, replacing the existing `list_roles/0` and `get_role/1`:
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul
git add lib/ex_cellence_server_web/live/members_live.ex test/ex_cellence_server_web/live/members_live_test.exs
...
Add to `test/ex_cellence_server_web/live/members_live_test.exs`:
...
Replace the `render/1` function in `lib/ex_cellence_server_web/live/members_live.ex`:
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix test test/ex_cellence_server_web/live/members_live_test.exs 2>&1 | tail -30
```
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix compile --warnings-as-errors 2>&1 | tail -20
```
...
describe "events" do
  ...
  test "save_member updates an existing built-in", %{conn: conn} do
    ...
    {:ok, rddef} =
      ExCellenceServer.Repo.insert(%ResourceDefinition{
        type: "role",
        name: "Grammar Editor",
        source: "code",
        status: "active",
    ...
    updated = ExCellenceServer.Repo.get!(ResourceDefinition, rddef.id)
  ...
  test "delete_member removes a custom member", %{conn: conn} do
    ...
    |> render_click(%{"id" => to_string(ExCellenceServer.Repo.get_by!(ResourceDefinition, name: "Deletable Role").id)})
...
Replace ALL existing `handle_event` implementations and update `mount/3` and `apply_action/3` in `lib/ex_cellence_server_web/live/members_live.ex`:
...
defp upsert_member_active(id, new_active, members) do
  ...
  if member && member.builtin do
    builtin = ExCellenceServer.Members.Member.get(id)
...
defp save_builtin_member(slug, params, members) do
  builtin = ExCellenceServer.Members.Member.get(slug)
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix test test/ex_cellence_server_web/live/members_live_test.exs 2>&1 | tail -40
```
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix compile --warnings-as-errors 2>&1 | tail -10 && mix format
```
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul && mix test 2>&1 | tail -20
```
...
**Files:**
- Modify: `lib/ex_cellence_server_web/router.ex`
...
```bash
grep -n "members" /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul/lib/ex_cellence_server_web/router.ex
```
...
```bash
cd /home/andrew/projects/ex_cellence_server/.worktrees/members-overhaul
git add lib/ex_cellence_server_web/router.ex
