defmodule ExCortex.PraxisTest do
  ...
  describe "all/0" do
    ...
    test "all templates have required fields" do
      for t <- Praxis.all() do
        ...
        assert is_map(t.rumination_definition), "#{t.id} missing rumination_definition"
      end
    end

    test "all template ids are unique" do
      ids = Enum.map(Praxis.all(), & &1.id)
  ...
  describe "by_category/1" do
    test "returns only templates for that category" do
      triage = Praxis.by_category(:triage)
      assert triage != []
  ...
  describe "all_with_status/0" do
    ...
    test "count matches all/0" do
      assert length(Praxis.all_with_status()) == length(Praxis.all())
    end
  ...
  describe "check_requirements/1" do
    test "returns list of {met, label} tuples" do
      ...
      # no requirements
      assert Praxis.check_requirements(template) == []
    end

    test "returns false for missing source type" do
      template = Praxis.get("jira_ticket_triage")
      results = Praxis.check_requirements(template)
  ...
  describe "readiness/1" do
    test "returns :ready for template with no requirements" do
      template = Praxis.get("incident_postmortem")
      assert Praxis.readiness(template) == :ready
    end

    test "returns :unavailable or :almost when requirements missing" do
      template = Praxis.get("jira_ticket_triage")
      # requires webhook source + slack expression — webhook source won't be configured
      assert Praxis.readiness(template) in [:unavailable, :almost]
    end
  end
end
